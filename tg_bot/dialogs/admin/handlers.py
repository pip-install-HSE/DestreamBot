...
async def menu(message: types.Message, user: dict, bot_user: BotUser):
    await message.answer(
        ...
        reply_markup=keyboards.main_menu(await bot_user.groups.all()))
...
async def new_chat_member(message: types.Message, state: FSMContext, bot_user: BotUser):
    ...
    group, _ = await Group.get_or_create(tg_id=message.chat.id, admin=bot_user)
...
async def my_group(callback: types.CallbackQuery, state: FSMContext, bot_user: BotUser):
    group_id = g_id if (g_id := callback.data.split(":")[-1]) else (await state.get_data()).get("group_id")
